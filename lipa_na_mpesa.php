                ...
                <div class="fooditem col-sm-6 col-md-3 mb-4">
                    ...
                    <div class="card">
                        ...
                        <form class="login-form" style="margin-top: 0px;" action="" method="post">
                            ...
                            <input type="date" name="date_in" id="date_in" class="form-control date_in" required onchange="checkin(this, this.value)">
                            <span class="label-input">Checkout</span>
                            <input type="date" name="date_out" id="date_out" class="form-control date_out" required onchange="checkout(this, this.value)">
            ...
            <div class="fooditem col-sm-6 col-md-3 mb-4">
                <div class="card">
                    ...
                    <form action="" method="post">
                        ...
                        <input type="hidden" name="room_price" value="<?php echo $rows["room_price"]; ?>">
                        <span class="label-input">Checkin</span>
                        <input type="date" name="date_in" class="form-control" required onchange="checkin(this.value)">
                        <span class="label-input">Checkout</span>
                        <input type="date" name="date_out" class="form-control" required onchange="checkout(this.value)">
                        <span class="label-input">No of Nights</span>
                        <input type="number" name="no_of_nights" class="form-control">
                        <div class="btn-wrap">
                            <button class="btn btn-warning" type="submit" name="book">Book Now</button>
...
<script>
    function checkin(e, value) {


        let date_in_element = document.getElementById("date_in");
        let dateoutvalue = e;
        console.log(dateoutvalue)
        while (dateoutvalue && !dateoutvalue.classList.contains("date_out")) { // check for existence and class
            dateoutvalue = dateoutvalue.nextElementSibling; // if it exists, but the class does not, move to the next element and repeat.
            console.log(1)
        }
        //  return dateoutvalue.value ; // return whatever was found, or null
        if (dateoutvalue.value.length == 0) {
            
            let date_in = new Date(value);
            let today = new Date().toLocaleDateString(); //javascript function to check today,s date
            today = new Date(today); // convert it to readable format

            if (date_in < today) { //check if date is in the past
                console.log("date must not be in the past because you have filled the next part");
            }
            
        }else{
            date_out = new Date(dateoutvalue.value)
             if (date_in > date_out) {
    ...
    function dateFunction(e) {
        ...
        let dateoutvalue = date_in.nextSibling.nextSibling.nextSibling.nextSibling.value;
        let dateinvalue = date_out.previousSibling.previousSibling.previousSibling.previousSibling.value;

        date_in = new Date(date_in.value)
        date_out = new Date(date_out.value)
        let today = new Date().toLocaleDateString()
        today = new Date(today);
        if (date_in < today && dateoutvalue == "") {
            console.log("date must not be in the past  because you have not filled the next field");
        } else if (date_in < today) {
            console.log("date must not be in the past because you have filled the next part");
        } else if (date_in > date_out) {
            console.log("wrong date");
        } else if (date_out < today && dateinvalue == "") {
            console.log("out date is in the");
        } else {
            console.log("wrong dateyyy");
            let diffTime = Math.abs(date_out - date_in);
        ...
        // if ( date_out.previousSibling.previousSibling.previousSibling.previousSibling.value != "")
        //  {
        //     // date_in.nextSibling.nextSibling.nextSibling.nextSibling.value != ""
        //     date_in = new Date(date_out.previousSibling.previousSibling.previousSibling.previousSibling.value)
        //     if (date_in.nextSibling.nextSibling.nextSibling.nextSibling.value != ""){
        //         date_out = new Date(date_out.previousSibling.previousSibling.previousSibling.previousSibling.value)
    ...
    // console.log(1)
    // console.log(date_in.nextSibling.nextSibling.nextSibling.nextSibling.value)
    // console.log(1)
</script>
