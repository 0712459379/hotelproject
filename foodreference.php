...
if (isset($_POST['submitorder'])) { //when the customer clicks the submit order button
  if (isset($_SESSION['cart'])) { //checks whether the cart has any existing food items
    ...
    { // if there isn't an order, make new order in orders table and save its id in session array
      ...
      $sql_order = "INSERT INTO foodorders (date, email,tel_no,order_total) VALUES (?,?,?,?)"; //insert in the foodorders tabel the date
      if($stmt = $mysqli->prepare($sql_order)){
        // Bind variables to the prepared statement as parameters
        $stmt->bind_param("isid",  $today, $user_email, $tel_no,$order_total);
        // Set parameters
        $today = $today;
        ...
        if($stmt->execute()){
            ...
            foreach ($_SESSION['cart'] as $item) { //loop through the session variable then put in food_order_items db
              ...
              if($stmt = $mysqli->prepare($sql_add_item)){
                // Bind variables to the prepared statement as parameters
                $stmt->bind_param("isisidd",  $order_id, $tel_no , $foodid , $foodname , $quantity , $foodprice ,  $foodprice_total );
                ...
                $quantity = $quantity;
                $foodprice = $foodprice;
                $foodprice_total = $foodprice_total;
                ...
                echo "Unable to insert user order";
            }
    ...
    unset($_SESSION['cart']);
    header("Location: payment.php?amount=$order_total");
  } else {
                  ...
                  <div class="container">
                    <form action="" method="post">
                      ...
                      <div class="row d-flex justify-content-between ">
                        ...
                        <div class="">
                          <button class="btn btn-success" type="submit" name="add">Order</button>
                        </div>
                      </div>
